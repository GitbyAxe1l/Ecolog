package dev.eministar.echolog.command;

import dev.eministar.echolog.EchoLogPlugin;
import dev.eministar.echolog.gui.MainSettingsGUI;
        ...
        if (args[0].equalsIgnoreCase("update")) {
            ...
            if (s instanceof Player p) {
                if (plugin.updates().isUpdateAvailable()) {
                    new dev.eministar.echolog.update.UpdateNotifyListener(plugin, plugin.updates())
                            .notifyPlayerNow(p);
                } else {
